...
while [ -z "${CTID_FROM:+x}" ]; do
  CTID_FROM=$(whiptail --backtitle "HomeLab YuxTec" --title "$TITLE" --radiolist \
    "\nWhich HA Podman LXC would you like to copy FROM?\n" \
    16 $(($MSG_MAX_LENGTH + 23)) 6 \
    "${CTID_MENU[@]}" 3>&1 1>&2 2>&3) || exit
done
while [ -z "${CTID_TO:+x}" ]; do
  CTID_TO=$(whiptail --backtitle "HomeLab YuxTec" --title "$TITLE" --radiolist \
...
whiptail --backtitle "HomeLab YuxTec" --defaultno --title "$TITLE" --yesno \
