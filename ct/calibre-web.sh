...
function update_script() {
  ...
  CHOICES=$(whiptail --backtitle "HomeLab YuxTec" --title "CALIBRE-WEB OPTIONS" --separate-output --checklist "Choose Additional Options" 15 125 8 "${menu_array[@]}" 3>&1 1>&2 2>&3)
  spinner &
  SPINNER_PID=$!
  if [ ! -z "$CHOICES" ]; then
    declare -a options
